...

{
	...
	for($safe_index = 0; $safe_index -lt $safes.count; $safe_index++){
		...
		$form.Text = "Confirm Safe $($safe.identifier)";
		$form.Size = vec2 500 500;
		...
		foreach($field in $safe.PsObject.Properties){
			...
			# Region (Combobox)
			if($field.name -eq "Region"){
				$combo_field = New-Object System.Windows.Forms.ComboBox;
				$combo_field.Location = vec2 $safe_center ($basey+4);
				$combo_field.Size = vec2 200 25;
				$combo_field.Text = $field.value;
				$combo_field.Items.Add("production") | Out-Null;
				$combo_field.Items.Add("production dr") | Out-Null;
				$combo_field.Items.Add("acceptance") | Out-Null;
				$combo_field.Items.Add("integration") | Out-Null;
				$combo_field.Items.Add("development") | Out-Null;
				$combo_field.Items.Add("other") | Out-Null;
				if($field.value -eq ""){
					$combo_field.BackColor = "Pink";
				}
				$combo_field.Name = "txt_" + $safe_index + "." + $field.name;
			...
			# Uneditable Textbox field
			} elseif($field.name -eq "Application"){
			...
			# Normal Textbox field
				...
				if($field.name -eq "Identifier"){
					$txt_field.MaxLength = 28; # limit identifier field length
				}
			}
...
#	$chk_enable.Checked = $true;
			$chk_enable.Checked = $global:migration_meta.selectall_ids;
		...
		$btn_add.Location = vec2 4 ($basey+4);
